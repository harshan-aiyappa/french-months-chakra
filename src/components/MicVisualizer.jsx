...
const MicVisualizer = ({ isListening, setIsSpeaking, setMicError, showToast }) => {
  const audioContextRef = useRef(null);
  const streamRef = useRef(null);
  const animationFrameRef = useRef(null);
  const calibrationTimerRef = useRef(null);

  // The 'useState' hook is now correctly imported and used.
  ...
  useEffect(() => {
    // This is the most robust and correct implementation.
    let analyser;
    let dataArray;
    let isCalibratingRef = true; // Use a ref for the calibration state inside the loop
    ...
    const calibrate = () => {
      ...
      isCalibratingRef = true;
      const noiseSamples = [];

      const calibrationLoop = () => {
        if (!isCalibratingRef) return;
    ...
    const visualize = (threshold) => {
      const update = () => {
        ...
        setIsSpeaking(isCurrentlySpeaking && isListening);
  ...
  }, [setMicError, setIsSpeaking, isListening, showToast, showToastOnce]);
