  ...
  useEffect(() => {
      ...
      if (window.micStream) {
        ...
        try {
          ...
          let errorDetails = {
            ...
            desc: "Could not access the microphone.",
          };
          if (err.name === "NotFoundError") {
            errorDetails = {
              id: "M-3",
              title: "No Microphone Found",
              desc: "Please connect a mic.",
            };
          } else if (err.name === "NotAllowedError") {
            errorDetails = {
              id: "M-2",
              title: "Access Denied",
              desc: "Please allow mic access.",
            };
          }
          setMicError(errorDetails.desc);
          showToastOnce(
            "error",
            `${errorDetails.title} (${errorDetails.id})`,
            errorDetails.desc
          );
          return;
        }
    ...
    const calibrate = () => {
      ...
      const finishCalibration = () => {
        ...
        const averageNoise =
          noiseSamples.length > 0
            ? noiseSamples.reduce((sum, val) => sum + val, 0) /
              noiseSamples.length
            : 10;
        ...
        showToast(
          ...
          `Threshold set to ${newThreshold.toFixed(0)}.`
        );
    ...
    const visualize = (threshold) => {
      const update = () => {
        ...
        for (let i = 0; i < NUM_BARS; i++) {
          ...
          const finalHeight =
            5 + baseHeight + (isCurrentlySpeaking && isListening ? 15 : 0);
    ...
    return () => {
      ...
      // Do not stop the shared stream tracks, only disconnect the context
    };
  }, [
    onCalibrationComplete,
    setMicError,
      ...
      <AnimatePresence>
        {barHeights.map((height, i) => {
          ...
          if (isCalibrating) {
            ...
            if (isThresholdMet) {
              boxShadow = `inset 0 6px 8px -2px ${indicatorColor}`;
            }
          }
          return (
            <MotionBar
